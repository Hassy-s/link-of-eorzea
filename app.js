(() => {
  ...
  function getPixelDifference(data, index, background) {
    const alpha = data[index + 3];
    if (alpha < 12) return 0;
  ...
  function getCanvasCornerBackground(context, width, height) {
    const points = [
      [0, 0],
    ...
    const total = points.reduce((result, [x, y]) => {
    ...
    return {
      ...
      alpha: total.alpha / points.length
    };
  }

  function findFaviconContentBounds(context, width, height) {
    const imageData = context.getImageData(0, 0, width, height);
    const data = imageData.data;
    const background = getCanvasCornerBackground(context, width, height);
    const threshold = 24;
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        if (getPixelDifference(data, index, background) <= threshold) continue;
    ...
    const padding = Math.max(2, Math.round(Math.max(contentWidth, contentHeight) * 0.035));
  ...
  function autoFitFaviconImage(image) {
    if (!image || image.dataset.faviconFitted === 'true') return;
    image.dataset.faviconFitted = 'true';
    ...
    try {
      ...
      const bounds = findFaviconContentBounds(sourceContext, sourceWidth, sourceHeight);

      const outputSize = 512;
      const safeSize = Math.round(outputSize * 0.95);
      const scale = Math.min(safeSize / bounds.width, safeSize / bounds.height);
      ...
      image.classList.add('favicon-auto-fitted');
    } catch (error) {
      // CORSなどで画像解析できない場合は、元のfavicon表示をそのまま利用する。
      console.debug('faviconの自動フィットをスキップしました。', error);
    }
  }
        ...
        <article class="link-card sortable-item" data-drop-type="link" data-category-id="${categoryId}" data-subcategory-id="${subcategoryId || ''}" data-link-id="${link.id}">
          ...
          <a class="card-link" href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(link.name)}を開く">
            <div class="card-image-wrap${faviconClass}">
              ${image.url ? `<img class="card-image${imageClass}" ${image.type === 'favicon' ? 'crossorigin="anonymous" onload="autoFitFaviconImage(this)"' : ''} src="${escapeHtml(image.url)}" alt="" loading="lazy" onerror="this.closest('.card-image-wrap').classList.add('image-error');this.remove();">` : ''}
            </div>
            <div class="card-body">
